...
export class NstLayoutStorage implements ILayoutStorage {
  ...
  public async list(namespace: string): Promise<readonly Layout[]> {
    ...
    const layoutFilePath = 'projects/peek-ai-2023/data/layoutDB.json'
    ...
    const layoutFileContents = (await (await fetch(nstLayoutUrl)).json()) as Record<
      string,
      unknown
    >;
